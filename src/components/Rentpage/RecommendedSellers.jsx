...
import { DUMMY_RECOMMENDED_SELLERS } from "../dummy/recommendedSellersDummy";
...
const RecommendedSellers = () => {
  ...
  const hasRealData = useMemo(() => {
    return Array.isArray(recommendedSellers) && recommendedSellers.length > 0;
  }, [recommendedSellers]);

  // ✅ if API fails or no data, show dummy silently
  const showDummy = !!error || !hasRealData;
  const listToRender = showDummy ? DUMMY_RECOMMENDED_SELLERS : recommendedSellers;
  ...
  useEffect(() => {
    ...
    container.addEventListener("scroll", updateArrowsVisibility, { passive: true });
  ...
  return (
    <section
      className="py-6 px-3 sm:px-4 md:px-8"
      style={{ backgroundColor: "var(--color-background)" }}
    >
      <div className="mb-6">
        <h2 className="text-2xl sm:text-3xl font-bold text-center text-[var(--color-foreground)]">
          Recommended Sellers
        </h2>
        <p className="text-sm sm:text-md text-center mt-1 text-[var(--color-text-secondary)]">
          Top brokers and agents
        </p>
              ...
              >
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {(property.title ? property.title.substring(0, 2) : "NA").toUpperCase()}
                    </div>
